...
function updateExtraFields(serviceValue) {
    if (serviceValue === 'Workshop' || serviceValue === 'Catering') {
        // Küchen-Check für beide anzeigen
        workshopDetails.style.display = 'block';

        // Ofen-Frage nur bei Workshop anzeigen
        if (serviceValue === 'Workshop') {
            ovenQuestion.style.display = 'block';
        } else {
            ovenQuestion.style.display = 'none';
        }
    } else {
        // Falls gar nichts gewählt ist
        workshopDetails.style.display = 'none';
    }
}
...
// Integration in die bestehende Modal-Öffnen-Logik
openButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
        e.preventDefault();
        const selectedService = btn.getAttribute('data-service');
